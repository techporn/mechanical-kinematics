/*
    反発ありの放物運動シミュレーション
    ２次のルンゲクッタ法（ホイン法）
 */

#include <stdlib.h>
#include "glut.h"

double mouse_x = 0, mouse_y = 1.5;
double dt=0.01;
int flag=0;
double e=0.8;
double M=1.0, c = 0.2;
double x1_k, x1_k1, x2_k, x2_k1, x3_k, x3_k1, x4_k, x4_k1, tim=0;
double x_k [4] , x_k1 [4];
double k1 [4] , k2 [4];


double func0 ( double x0 , double x1)
{
    return x1;
}

double func1 ( double x0 , double x1)
{
    return -c/M*x1;
}

double func2 ( double x0 , double x1)
{
    return x1;
}

double func3 ( double x0 , double x1)
{
    return -c*x1/M - 9.8;
}

void display(void)
{
	glClear(GL_COLOR_BUFFER_BIT);
	glColor3d(1.0, 1.0, 1.0);
	glBegin(GL_LINES);
		glVertex2d(0, 0);
		glVertex2d(7.5, 0);
		glVertex2d(0, 0);
		glVertex2d(0, 4.0);
	glEnd();
	glColor3d(1.0, 1.0, 0.0);
	glPointSize(10);
		glBegin(GL_POINTS);
		glVertex2d(x_k[0], x_k[2]);
	glEnd();

	glutSwapBuffers();
}

void keyboard(unsigned char key, int x, int y)
{
	if(key == 27) exit(0);
}

void mouse(int button, int state, int x, int y)
{
	if(state == GLUT_DOWN)
	{
		mouse_x = -0.5 + (7.5-(-0.5))/800*(double)x;
		mouse_y =  4.5 - (4.5-(-0.5))/500*(double)y;
		flag = 0;
		x_k[0] = 0;
		x_k[1] = 5;
		x_k[2] = 0;
		x_k[3] = 5;
		glutPostRedisplay();
	}
	else
	{
		flag = 1;
	}
}

void timer(int id)
{
	glutTimerFunc(dt*1000, timer, 0);
	if(flag == 1)
	{
		// x1_k1 = x1_k + x2_k*dt;
		// x2_k1 = x2_k + (-c/M*x2_k)*dt;
		// x3_k1 = x3_k + x4_k*dt;
		// x4_k1 = x4_k + (-c/M*x4_k - 9.8)*dt;


        k1[0] = func0 (x_k [0] , x_k [1]) *dt;
        k1[1] = func1 (x_k [0] , x_k [1]) *dt;
        k2[0] = func0 (x_k [0] + k1 [0] , x_k[1] + k1[1]) *dt;
        k2[1] = func1 (x_k [0] + k1 [0] , x_k[1] + k1[1]) *dt;
        x_k1[0] = x_k [0] + (k1 [0] + k2 [0]) / 2;
        x_k1[1] = x_k [1] + (k1 [1] + k2 [1]) / 2;
        x_k[0] = x_k1 [0];
        x_k[1] = x_k1 [1];

        k1[2] = func2 (x_k [2] , x_k [3]) *dt;
        k1[3] = func3 (x_k [2] , x_k [3]) *dt;
        k2[2] = func2 (x_k [2] + k1 [2] , x_k[3] + k1[3]) *dt;
        k2[3] = func3 (x_k [2] + k1 [2] , x_k[3] + k1[3]) *dt;
        x_k1[2] = x_k [2] + (k1 [2] + k2 [2]) / 2;
        x_k1[3] = x_k [3] + (k1 [3] + k2 [3]) / 2;
        x_k[2] = x_k1 [2];
        x_k[3] = x_k1 [3];

		// 反発処理
		if(x_k[2]<= 0)
		{
      		x_k[2] = 0;
			x_k[3] = -e * x_k[3];
		}
	}
	glutPostRedisplay();
}

void init(void)
{
	glClearColor(0.0, 0.0, 0.0, 1.0);
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	glOrtho(-0.5, 7.5, -0.5, 4.5, -1.0, 1.0);
}

int main(int argc, char** argv)
{
	glutInit(&argc, argv);
	glutInitDisplayMode(GLUT_RGBA | GLUT_DOUBLE);
	glutInitWindowSize(800, 500);
	glutInitWindowPosition(0, 0);
	glutCreateWindow(argv[0]);
	glutDisplayFunc(display);
	glutKeyboardFunc(keyboard);
	glutMouseFunc(mouse);
	init();
	glutTimerFunc(dt*1000, timer, 0);
	glutMainLoop();
	return 0;
}

...
void display(void)
{
	...
	glPushMatrix();
        ...
        glPushMatrix();
            // Arm2についての変形
            // glTranslated(ARM1_LENGTH*sin(theta1), 1.0, cos(theta1));  // Arm1の長さだけ移動する
            glRotated(theta1, 0, 1, 0);    				// theta 1だけ回転する
            glTranslated(ARM1_LENGTH, 1.0, 0);  		// Arm1の長さ端を原点にする

            glRotated(theta2, 0, 1, 0);    			  	// theta 2だけ回転する
            glTranslated((ARM2_LENGTH)*0.5, 0.0, 0);  	// 原点中心に引き延ばされるので、1/2だけ移動して物体の端を原点にする

            glScaled(ARM2_LENGTH , 1.0, 1.0);      		// x方向に引き延ばす
